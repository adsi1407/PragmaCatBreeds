...
void main() {
  group('SearchCatBreedsUseCase', () {
    ...
    group('call |', () {
      test('repositoryReturnsMatchingBreeds | validQuery | returnsFilteredBreeds', () async {
        // Arrange
        const query = 'Persian';
        ...
        when(() => mockRepository.searchCatBreeds(query))
      ...
      test('repositoryReturnsMultipleMatches | validQuery | returnsAllMatches', () async {
        // Arrange
        const query = 'a';
        final expectedBreeds = [
        ...
        expect(result.every((breed) => breed.name.toLowerCase().contains('a')), isTrue);
        verify(() => mockRepository.searchCatBreeds(query)).called(1);
      });

      test('repositoryReturnsNoMatches | validQuery | returnsEmptyList', () async {
        // Arrange
        const query = 'XYZ';
      ...
      test('repositoryReceivesEmptyQuery | emptyString | callsRepositoryWithEmptyString', () async {
        // Arrange
        const query = '';
        final expectedBreeds = [
          CatBreedTestDataBuilder.persian().build(),
      ...
      test('repositoryReceivesWhitespaceQuery | whitespaceString | passesQueryAsIs', () async {
        // Arrange
        const query = '   ';
        const expectedBreeds = <CatBreed>[];
      ...
      test('repositoryReceivesCaseVariations | caseInsensitiveQuery | passesQueryAsProvided', () async {
        // Arrange
        ...
        when(() => mockRepository.searchCatBreeds(upperQuery))
            .thenAnswer((_) async => expectedBreeds);
        when(() => mockRepository.searchCatBreeds(lowerQuery))
            .thenAnswer((_) async => expectedBreeds);
        when(() => mockRepository.searchCatBreeds(mixedQuery))
            .thenAnswer((_) async => expectedBreeds);
      ...
      test('repositoryReceivesSpecialCharacters | specialCharQuery | passesQueryAsProvided', () async {
        // Arrange
      ...
      test('repositoryReceivesLongQuery | longString | handlesLongQuery', () async {
        // Arrange
      ...
      test('multipleConsecutiveSearches | differentQueries | callsRepositoryForEach', () async {
        // Arrange
        ...
        when(() => mockRepository.searchCatBreeds(firstQuery))
            .thenAnswer((_) async => firstResult);
        when(() => mockRepository.searchCatBreeds(secondQuery))
      ...
      test('repositorySameQueryMultipleTimes | sameQuery | callsRepositoryEachTime', () async {
        // Arrange
        const query = 'Persian';
        final expectedBreeds = [CatBreedTestDataBuilder.persian().build()];
