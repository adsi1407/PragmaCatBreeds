...
void main() {
  group('CatBreedRepositoryProxy', () {
    ...
    group('getCatBreeds', () {
      ...
      test('getCatBreeds | cacheMiss | fetchesFromRepositoryAndCaches', () async {
        // Arrange
        when(() => mockCache.get('all_breeds')).thenReturn(null);
        when(() => mockRepository.getCatBreeds()).thenAnswer((_) async => testBreeds);
      ...
      test('getCatBreeds | repositoryReturnsEmpty | handlesEmptyList', () async {
        // Arrange
        const emptyList = <CatBreed>[];
        when(() => mockCache.get('all_breeds')).thenReturn(null);
        when(() => mockRepository.getCatBreeds()).thenAnswer((_) async => emptyList);
      ...
      test('getCatBreeds | repositoryThrows | propagatesException', () async {
        // Arrange
        when(() => mockCache.get('all_breeds')).thenReturn(null);
        when(() => mockRepository.getCatBreeds()).thenThrow(Exception('API Error'));
    ...
    group('searchCatBreeds', () {
      ...
      test('searchCatBreeds | cacheMiss | fetchesAndCachesResults', () async {
        ...
        when(() => mockRepository.searchCatBreeds(query)).thenAnswer((_) async => expectedResults);
        when(() => mockCache.put('search:abyssinian', expectedResults)).thenReturn(null);
        ...
        verify(() => mockCache.put('search:abyssinian', expectedResults)).called(1);
      });

      test('searchCatBreeds | queryWithWhitespace | normalizesForCacheKey', () async {
        // Arrange
        const query = '  PERSIAN  ';
        final expectedResults = [testBreed2];
        when(() => mockCache.get('search:persian')).thenReturn(null);
        when(() => mockRepository.searchCatBreeds(query)).thenAnswer((_) async => expectedResults);
        when(() => mockCache.put('search:persian', expectedResults)).thenReturn(null);
        ...
        verify(() => mockCache.get('search:persian')).called(1);
        verify(() => mockRepository.searchCatBreeds(query)).called(1);
        verify(() => mockCache.put('search:persian', expectedResults)).called(1);
      });
      ...
      test('searchCatBreeds | noResults | handlesEmptyResults', () async {
        ...
        when(() => mockRepository.searchCatBreeds(query)).thenAnswer((_) async => emptyResults);
        when(() => mockCache.put('search:nonexistent', emptyResults)).thenReturn(null);
        ...
        verify(() => mockRepository.searchCatBreeds(query)).called(1);
        verify(() => mockCache.put('search:nonexistent', emptyResults)).called(1);
      });

      test('searchCatBreeds | queryWithSpecialChars | handlesSpecialCharacters', () async {
        // Arrange
        const query = 'cat@breed#123!';
        const expectedCacheKey = 'search:cat@breed#123!';
        when(() => mockCache.get(expectedCacheKey)).thenReturn(null);
        when(() => mockRepository.searchCatBreeds(query)).thenAnswer((_) async => []);
      ...
      test('searchCatBreeds | apiThrowsException | propagatesException', () async {
        // Arrange
        const query = 'persian';
        when(() => mockCache.get('search:persian')).thenReturn(null);
        when(() => mockRepository.searchCatBreeds(query)).thenThrow(Exception('Search API Error'));
    ...
    group('getCatBreedById', () {
      test('getCatBreedById | breedInAllBreedsCache | returnsFromCache', () async {
        // Arrange
        const breedId = 'abys';
        when(() => mockCache.get('all_breeds')).thenReturn(testBreeds);
      ...
      test('getCatBreedById | breedInSpecificCache | returnsFromSpecificCache', () async {
        // Arrange
        const breedId = 'abys';
        when(() => mockCache.get('all_breeds')).thenReturn(null);
      ...
      test('getCatBreedById | notInCache | fetchesFromRepositoryAndCaches', () async {
        // Arrange
        const breedId = 'abys';
        when(() => mockCache.get('all_breeds')).thenReturn(null);
        when(() => mockCache.get('breed:abys')).thenReturn(null);
        when(() => mockRepository.getCatBreedById(breedId)).thenAnswer((_) async => testBreed1);
        when(() => mockCache.put('breed:abys', [testBreed1])).thenReturn(null);
      ...
      test('getCatBreedById | breedNotFound | returnsNullWithoutCaching', () async {
        // Arrange
        const breedId = 'nonexistent';
        when(() => mockCache.get('all_breeds')).thenReturn(null);
        when(() => mockCache.get('breed:nonexistent')).thenReturn(null);
        when(() => mockRepository.getCatBreedById(breedId)).thenAnswer((_) async => null);
      ...
      test('getCatBreedById | breedNotInAllBreedsCache | returnsNull', () async {
        // Arrange
        const breedId = 'nonexistent';
        when(() => mockCache.get('all_breeds')).thenReturn(testBreeds);
        when(() => mockCache.get('breed:nonexistent')).thenReturn(null);
      ...
      test('getCatBreedById | emptyCacheList | fetchesFromRepository', () async {
        // Arrange
        const breedId = 'abys';
        when(() => mockCache.get('all_breeds')).thenReturn(null);
      ...
      test('getCatBreedById | repositoryThrows | propagatesException', () async {
        // Arrange
        const breedId = 'abys';
        when(() => mockCache.get('all_breeds')).thenReturn(null);
        when(() => mockCache.get('breed:abys')).thenReturn(null);
        when(() => mockRepository.getCatBreedById(breedId)).thenThrow(Exception('Repository Error'));
    ...
    group('integration scenarios', () {
      test('getCatBreeds | cacheHitThenMiss | handlesSequentialScenarios', () async {
        // Arrange - First call hits cache, second call after clear misses
        when(() => mockCache.get('all_breeds'))
        ...
        verify(() => mockRepository.getCatBreeds()).called(1); // Only called once for miss
        verify(() => mockCache.put('all_breeds', testBreeds)).called(1);
        verify(() => mockCache.clear()).called(1);
      });

      test('getCatBreeds | concurrentCalls | handlesConcurrentOperations', () async {
        // Arrange
        when(() => mockCache.get('all_breeds')).thenReturn(null);
        when(() => mockRepository.getCatBreeds()).thenAnswer((_) async => testBreeds);
        when(() => mockCache.put('all_breeds', testBreeds)).thenReturn(null);
        ...
        final futures = List.generate(3, (_) => repositoryProxy.getCatBreeds());
      ...
      test('searchCatBreeds | queryNormalization | maintainsCacheKeyConsistency', () async {
        // Arrange
        const searchQuery = 'Test Query';
        const normalizedKey = 'search:test query';
        
        when(() => mockCache.get(normalizedKey)).thenReturn(null);
        when(() => mockRepository.searchCatBreeds(searchQuery)).thenAnswer((_) async => []);
